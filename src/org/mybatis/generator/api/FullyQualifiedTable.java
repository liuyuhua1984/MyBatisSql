...
public class FullyQualifiedTable {
	 ...
	 *
	 * @param introspectedCatalog the actual catalog of the table as returned from DatabaseMetaData. This value should only be set if the user configured a catalog. Otherwise the DatabaseMetaData
	 *                is reporting some database default that we don't want in the generated code.
	 ...
	 * @param ignoreQualifiersAtRuntime if true, then the catalog and schema qualifiers will be ignored when composing fully qualified names in the generated SQL. This is used, for example, when
	 *                the user needs to specify a specific schema for generating code but does not want the schema in the generated SQL
	 * @param runtimeCatalog this is used to "rename" the catalog in the generated SQL. This is useful, for example, when generating code against one catalog that should run with a different
	 *                catalog.
	 ...
	 * @param runtimeTableName this is used to "rename" the table in the generated SQL. This is useful, for example, when generating code to run with an Oracle synonym. The user would have to
	 *                specify the actual table name and schema for generation, but would want to use the synonym name in the generated SQL
	 ...
	 * Calculates a Java package fragment based on the table catalog and schema. If qualifiers are ignored, then this method will return an empty string.
	 * 
	 * This method is used for determining the sub package for Java model objects only. It takes into account the possibility that a sub-package was added to the domain object name in the table
	 * configuration.
	 *
	 * @param isSubPackagesEnabled the is sub packages enabled
	 * @return the subpackage for this table
